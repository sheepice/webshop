...
class GoodsGroup(BaseModel):
    ...
    image = models.ImageField(verbose_name='分类图标', blank=True, null=True)
...
class Goods(BaseModel):
    """商品"""
    group = models.ForeignKey('GoodsGroup', verbose_name='分类', help_text='分类', max_length=20, on_delete=models.CASCADE)
    ...
    cover = models.ImageField(verbose_name='封面图连接', help_text='封面图连接', blank=True, null=True)
...
class GoodsBanner(BaseModel):
    ...
    image = models.ImageField(verbose_name='轮播图链接', help_text='轮播图名称', blank=True, null=True)
